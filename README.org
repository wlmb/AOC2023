...
my $distance = compute_distance({%graph}, "$start D", "$end D");
say $distance;
...
#+begin_src perl  :tangle 23c.pl :shebang #!/usr/bin/env perl
use v5.36;
use List::AllUtils;
use PDL;
my %directions=(D=>pdl([0,1]), L=>pdl([-1,0]), U=>pdl([0,-1]), R=>pdl([1,0]));
...
my %graph;
push $graph{"$_->[0]"}->@*, ["$_->[1]", $_->[2]] for @graph; # from=>[[to, distance],...]
push $graph{"$_->[1]"}->@*, ["$_->[0]", $_->[2]] for @graph; # from=>[[to, distance],...]
delete $graph{"$end"};
my $distance = compute_distance({%graph}, "$start", "$end");
say $distance;
